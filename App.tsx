...
import { BookOpenIcon, QuestionMarkCircleIcon, InformationCircleIcon, HamburgerIcon } from './components/IconComponents';
import GuideModal from './components/GuideModal';
import SourcesModal from './components/SourcesModal';
import SiteNav from './components/SiteNav';
import Translator from './components/Translator';

type AppView = 'dictionary' | 'translator';
...
const App: React.FC = () => {
  ...
  const [currentView, setCurrentView] = useState<AppView>('dictionary');
  ...
  return (
    <div className="min-h-screen bg-gray-100 dark:bg-slate-900 font-sans text-gray-900 dark:text-gray-200 p-4 sm:p-6 lg:p-8">
       <SiteNav 
        ...
        onOpenAbbreviations={() => { setIsNavOpen(false); setIsGuideModalOpen(true); }}
        onOpenSources={() => { setIsNavOpen(false); setIsSourcesModalOpen(true); }}
      />
      <div className="container mx-auto max-w-4xl">
        <header className="text-center mb-8">
           <div className="absolute top-0 left-0">
             <button 
                ...
                className="p-2 rounded-full text-gray-500 hover:bg-gray-200 dark:text-gray-400 dark:hover:bg-slate-700 focus:outline-none focus:ring-2 focus:ring-blue-500"
                aria-label="Open site menu"
                aria-haspopup="dialog"
             >
                <HamburgerIcon className="h-7 w-7 sm:h-8 sm:w-8" />
             </button>
          </div>
        ...
        <main>
            {currentView === 'dictionary' && renderDictionaryView()}
